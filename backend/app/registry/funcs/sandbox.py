...
@func(description="在沙箱中安装 Python 包。run_code 报 ModuleNotFoundError 时用此工具安装缺失包",
     ...
     output_model=PipInstallOutput,
)
async def pip_install(packages: str) -> dict:
    """在沙箱中 pip install，安装到持久化卷，后续 run_code 可用。"""
    if not packages.strip():
        return {"result": "未指定要安装的包"}
    ...
    return {"result": f"{result['stdout'].strip()}，已安装：{packages}"}
...
async def run_code(code: str, timeout: int = 60) -> dict:
